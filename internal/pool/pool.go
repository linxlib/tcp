// Package pool provides object-reusable concurrent-safe pool.
package pool
...
func New(ttl time.Duration, newFunc NewFunc, expireFunc ...ExpireFunc) *Pool {
	...
	atimer.AddSingleton(time.Second, r.checkExpireItems)
...
func (p *Pool) Put(value interface{}) error {
	...
	if p.TTL == 0 {
		...
		item.expire = time.Now().UnixNano() + p.TTL.Nanoseconds()
	}
...
func (p *Pool) Get() (interface{}, error) {
	for !p.closed.Val() {
		if r := p.list.PopFront(); r != nil {
			f := r.(*poolItem)
			if f.expire == 0 || f.expire > time.Now().UnixNano() {
				return f.value, nil
			}
		} else {
			break
...
func (p *Pool) checkExpireItems() {
	...
	var timestampMilli = time.Now().UnixNano()
	for {
