...
func NewConn(addr string, timeout ...time.Duration) (*Connection, error) {
...
func NewConnTLS(addr string, tlsConfig *tls.Config) (*Connection, error) {
...
func NewConnKeyCrt(addr, crtFile, keyFile string) (*Connection, error) {
...
func NewConnByNetConn(conn net.Conn) *Connection {
	return &Connection{
		Conn:           conn,
...
func (c *Connection) Send(data []byte, retry ...Retry) error {
	for {
...
func (c *Connection) Recv(length int, retry ...Retry) ([]byte, error) {
...
func (c *Connection) RecvLine(retry ...Retry) ([]byte, error) {
	var err error
	var buffer []byte
	data := make([]byte, 0)
	for {
...
func (c *Connection) RecvTil(til []byte, retry ...Retry) ([]byte, error) {
	var err error
	var buffer []byte
	data := make([]byte, 0)
	length := len(til)
...
func (c *Connection) RecvWithTimeout(length int, timeout time.Duration, retry ...Retry) (data []byte, err error) {
...
func (c *Connection) SendWithTimeout(data []byte, timeout time.Duration, retry ...Retry) (err error) {
...
func (c *Connection) SendRecv(data []byte, length int, retry ...Retry) ([]byte, error) {
...
func (c *Connection) SendRecvWithTimeout(data []byte, length int, timeout time.Duration, retry ...Retry) ([]byte, error) {
	if err := c.Send(data, retry...); err == nil {
...
func (c *Connection) SetRecvDeadline(t time.Time) error {
...
func (c *Connection) SetSendDeadline(t time.Time) error {
...
func (c *Connection) SetRecvBufferWait(bufferWaitDuration time.Duration) {
